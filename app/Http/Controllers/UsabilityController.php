...
class UsabilityController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $respuesta = $http -> post('usabilities',$body);
    ...
    {
        $http = new Http();
        $respuesta = $http -> get('usabilities/'.$id);
        if($respuesta->res=='true'){
            return view('usabilities.edit',compact('respuesta','id'));
        }else{
    ...
    public function update(Request $request, string $id)
    {
        ...
        $respuesta = $http -> upd('usabilities/'.$id,$body);
        if($respuesta->res=='true'){
            $success = $respuesta -> msg;
            return redirect()->route('usabilities.index')->with('success',$success);
        }else{
    ...
    {
        $http = new Http();
        $respuesta = $http -> del('usabilities/'.$id);
        if($respuesta->res=='true'){
            $success = $respuesta->msg;
            return redirect()->route('usabilities.index')->with('success',$success);
        }else{
